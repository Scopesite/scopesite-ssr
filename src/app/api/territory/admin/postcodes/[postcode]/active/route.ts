...
import { updateTerritoryActive } from '@/lib/territory/queries';
...
export async function POST(
  ...
  try {
    const ok = await updateTerritoryActive(postcode, parsed.data.isActive);
    if (!ok) {
      return NextResponse.json({ error: 'Not found' }, { status: 404 });
    }
    await writeAuditLog({
      actionType: 'postcode_active_change',
      entityId: postcode,
    ...
    console.error('[admin/postcodes/active]', err);
